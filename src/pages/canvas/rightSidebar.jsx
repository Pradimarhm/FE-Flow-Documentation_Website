    ...
    return (
        <div className="w-full h-full bg-olive-50 border-l-2 border-olive-900 flex flex-col overflow-y-auto">
            <div className="p-4 border-b-4 border-olive-900 bg-olive-200 flex justify-between items-center">
                <div>
                    <h2 className="font-black text-lg text-olive-900 uppercase">Config</h2>
                    <p className="text-xs font-bold text-olive-700">ID: {selectedNode.id}</p>
                </div>
                <div
            ...
            <div className="p-4 flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Node Label</label>
                    <input
                        type="text" value={formData.label || ""}
                        onChange={(e) => handleRootChange("label", e.target.value)}
                        className="p-2 border-2 border-olive-900 text-sm font-semibold outline-none"
                    />
                </div>

                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Node Category</label>
                    <select
                ...
                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Input Params (JSON)</label>
                    <textarea
                        rows={3}
                        value={typeof formData.config?.inputParams === 'object' ? JSON.stringify(formData.config.inputParams, null, 2) : (formData.config?.inputParams || "")}
                        onChange={(e) => handleConfigChange("inputParams", e.target.value)}
                        className="p-2 border-2 border-olive-900 font-mono text-xs outline-none"
                    />
                </div>

                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Validation Logic</label>
                    <input
                        type="text" value={formData.config?.validationRules || ""}
                        onChange={(e) => handleConfigChange("validationRules", e.target.value)}
                        className="p-2 border-2 border-olive-900 text-xs outline-none"
                    />
                </div>

                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Process Logic</label>
                    <input
                        type="text" value={formData.config?.processLogic || ""}
                        onChange={(e) => handleConfigChange("processLogic", e.target.value)}
                        className="p-2 border-2 border-olive-900 text-xs outline-none"
                    />
                </div>

                <div className="flex flex-col gap-1">
                    <label className="text-xs font-bold text-olive-900 uppercase">Description</label>
                    <textarea
                        rows={2}
